...
** Check the `ConnectionStrings` in `appsettings.json` files under the `Libary.Blazor` and `Libary.DbMigrator` projects and change it if you need.
**
...
* Run `Libary.DbMigrator` to create the initial database. This should be done in the first run. It is also needed if a new database migration is added to the solution later.
...
* `Libary.DbMigrator`: A console application which applies the migrations and also seeds the initial data. It is useful on development as well as on production environment.
** `Libary.Blazor`: ASP.NET Core Blazor Server application that is the essential web application of the solution.
